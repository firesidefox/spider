...
```
spd host add --name web-01 --ip 1.2.3.4 --auth-type password --credential "xxx"
  │
  ▼
...
`spd` 不通过 HTTP API，而是直接访问本地 SQLite。这简化了本地开发和调试场景，不需要 spider 服务进程运行。代价是 spd 只能在与 spider 同机运行时使用。
