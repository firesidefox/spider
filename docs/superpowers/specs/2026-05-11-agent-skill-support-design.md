...
| description > 250 字符的 skill 上传 | UI 显示 ⚠，上传被拒，返回明确错误 |
| frontmatter YAML 格式错误 | UI 显示 ✗，错误消息含行号 |
| 删除 skill | 下一个 user turn skill 列表不含该项 |
| 新增 skill（对话进行中） | 下一个 user turn skill 列表含新项，含 supersedes 标记 |
| 8KB 预算超限 | 三级降级生效：先截断 description，极端情况只显示 name |
| 同一 turn 内已有 `<loaded-skill name=X>` | 模型不再调用 `invoke_skill("X")`，直接按指令执行 |
